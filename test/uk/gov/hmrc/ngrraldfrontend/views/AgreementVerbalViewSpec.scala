...
class AgreementVerbalViewSpec extends ViewBaseSpec {
  ...
  "AgreementVerbalView" must {
    val agreementVerbalView = view(content, form, radio, address, NormalMode)
    lazy implicit val document: Document = Jsoup.parse(agreementVerbalView.body)
    val htmlApply = view.apply(content, form, radio, address, NormalMode).body
    val htmlRender = view.render(content, form, radio, address, NormalMode, request, messages, mockConfig).body
    lazy val htmlF = view.f(content, form, radio, address, NormalMode)
