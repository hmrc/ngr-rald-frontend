...
class AboutTheRentFreePeriodViewSpec extends ViewBaseSpec {
  lazy val view: AboutTheRentFreePeriodView = inject[AboutTheRentFreePeriodView]

  object Strings {
    val heading = "About the rent-free period"
    val title = s"$heading - GOV.UK"
    val label1 = "How many months in the rent-free period?"
    val label2 = "What was the rent period start date?"
