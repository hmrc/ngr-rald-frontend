...
class ConfirmBreakClauseViewSpec extends ViewBaseSpec {
  lazy val view: ConfirmBreakClauseView = inject[ConfirmBreakClauseView]

  object Strings {
    val heading = "Did your agreement allow you to trigger a break clause?"
    val hint = "A break clause is a term in your agreement that allows you to end the agreement early"
    val radio1 = "Yes"
    val radio2 = "No"
  ...
  private val ngrRadio: NGRRadio = NGRRadio(NGRRadioName("confirmBreakClause-radio-value"),ngrTitle = Some(Legend(content = Text(messages("confirmBreakClause.title")), classes = "govuk-fieldset__legend--l", isPageHeading = true)) ,NGRRadioButtons = Seq(yesButton, noButton))
  val form = ConfirmBreakClauseForm.form.fillAndValidate(ConfirmBreakClauseForm("Yes"))
  val radio: Radios = buildRadios(form, ngrRadio)

  "ConfirmBreakClauseView" must {
    val confirmBreakClauseView = view(address, form, radio, NormalMode)
    lazy implicit val document: Document = Jsoup.parse(confirmBreakClauseView.body)
