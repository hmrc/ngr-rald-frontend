...
class MoneyToTakeOnTheLeaseViewSpec extends ViewBaseSpec {
  lazy val view: MoneyToTakeOnTheLeaseView = inject[MoneyToTakeOnTheLeaseView]

  object Strings {
    val heading = "Money you got from the landlord or previous tenant to take on the lease"
    val title = s"$heading - GOV.UK"
    val label1 = "How much money did you get to take on the lease (excluding VAT)?"
    val label2 = "When did you get the money?"
  ...
  val form = MoneyToTakeOnTheLeaseForm.form.fillAndValidate(MoneyToTakeOnTheLeaseForm(amount = BigDecimal(1000), date = NGRDate(day = "1", month = "1", year = "2020")))
  
  "MoneyToTakeOnTheLeaseView" must {
    val moneyYouPaidInAdvanceToLandlordView = view(form, address, NormalMode)
