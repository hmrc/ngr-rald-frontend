...
class NameSpec extends TestSupport {
  "Name" should {
    ...
    val name = Name("Josh")
    val json = Json.parse("""{ "value": "Josh" }""")
    ...
    "deserialize from JSON" in {
      json.validate[Name] mustBe JsSuccess(name)
    }

    "fail to deserialize from incorrect JSON" in {
      val invalidJson = Json.parse("""{ "wrongKey": "Josh" }""")
      invalidJson.validate[Name].isError mustBe true
    }
  }
}
