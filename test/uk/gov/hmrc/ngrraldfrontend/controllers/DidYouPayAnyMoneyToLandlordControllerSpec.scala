...
import uk.gov.hmrc.ngrraldfrontend.pages.{DidYouPayAnyMoneyToLandlordPage, DoesYourRentIncludeParkingPage}
...
class DidYouPayAnyMoneyToLandlordControllerSpec extends ControllerSpecSupport {
  ...
  val didYouPayAnyMoneyToLandlordAnswers: Option[UserAnswers] =  userAnswers.set(DidYouPayAnyMoneyToLandlordPage, true).toOption
  ...
  "didYouPayAnyMoneyToLandlordAnswers Controller" must {
    ...
    "method submit" must {
      ...
      "Return Form with Errors when no radio selection is input" in {
        val result = controllerProperty(None).submit(NormalMode)(AuthenticatedUserRequest(FakeRequest(routes.DidYouPayAnyMoneyToLandlordController.submit(NormalMode))
          .withFormUrlEncodedBody(
            "didYouPayAnyMoneyToLandlord-radio-value" -> "",
          )
          .withHeaders(HeaderNames.authorisation -> "Bearer 1"), None, None, None, Some(property), credId = Some(credId.value), None, None, nino = Nino(true, Some(""))))
        result.map(result => {
        ...
        val content = contentAsString(result)
        content must include(pageTitle)
        content must include("Select yes if you paid any money in advance to the landlord")
      }
