  ...
  "RepairsAndFittingOutController" must {
    ...
    "method submit" must {
      "Return See_Other and the correct view after submitting yes" in {
        ...
        result.map(result => {
          result.header.headers.get("Location") mustBe Some("/ngr-rald-frontend/about-repairs-and-fitting-out") 
        })
        status(result) mustBe SEE_OTHER
        redirectLocation(result) mustBe Some(routes.AboutRepairsAndFittingOutController.show(NormalMode).url)
      }
