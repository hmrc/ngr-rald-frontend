...
class HasAnythingElseAffectedTheRentControllerSpec extends ControllerSpecSupport {
  val pageTitle = "Has anything else affected the rent?"
  val view: HasAnythingElseAffectedTheRentView = inject[HasAnythingElseAffectedTheRentView]
  val controllerProperty: Option[UserAnswers] => HasAnythingElseAffectedTheRentController = answers => new HasAnythingElseAffectedTheRentController(view, mockAuthJourney, mockNGRCharacterCountComponent,fakeDataProperty(Some(property),answers), mockCheckRequestSentReference, mockNavigator, mockSessionRepository, mcc)(mockConfig, ec)
  val controllerNoProperty: HasAnythingElseAffectedTheRentController = new HasAnythingElseAffectedTheRentController(view, mockAuthJourney, mockNGRCharacterCountComponent, fakeData(None), mockCheckRequestSentReference, mockNavigator, mockSessionRepository, mcc)(mockConfig, ec)
  ...
  "What is your rent based on controller" must {
    "method show" must {
      ...
      "Return OK and the correct view with prepopulated data" in {
        ...
        document.select("input[type=radio][name=hasAnythingElseAffectedTheRent][value=true]").hasAttr("checked") mustBe true
        document.select("textarea[name=reason]").text() mustBe "The area has gone up in value"
    ...
    "method submit" must {
      "Return SEE_OTHER and redirect MoneyYouPaidInAdvanceToLandlord view when radio button selected no" in {
      ...
      "Return Form with Errors when no radio button is selected" in {
        val result = controllerProperty(None).submit(NormalMode)(AuthenticatedUserRequest(FakeRequest(routes.WhatIsYourRentBasedOnController.submit(NormalMode))
          .withFormUrlEncodedBody(
            "hasAnythingElseAffectedTheRent" -> ""
          )
          .withHeaders(HeaderNames.authorisation -> "Bearer 1"), None, None, None, Some(property), credId = Some(credId.value), None, None, nino = Nino(true, Some(""))))
        ...
        val content = contentAsString(result)
        content must include(pageTitle)
        content must include("Select yes if anything else has affected the rent.")
      }
      "Return Form with Errors when radio button Other is selected but no other description is given" in {
        ...
        val content = contentAsString(result)
        content must include(pageTitle)
        content must include("Tell us what else has affected your rent")
      }
      ...
      "Return Exception if no address is in the mongo" in {
        val exception = intercept[NotFoundException] {
          await(controllerNoProperty.submit(NormalMode)(AuthenticatedUserRequest(FakeRequest(routes.WhatIsYourRentBasedOnController.submit(NormalMode))
            .withFormUrlEncodedBody(("hasAnythingElseAffectedTheRent" -> "Other"))
            .withHeaders(HeaderNames.authorisation -> "Bearer 1"), None, None, None, Some(property), credId = Some(credId.value), None, None, nino = Nino(true, Some("")))))
        }
        exception.getMessage contains "Could not find answers in backend mongo" mustBe true
      }
