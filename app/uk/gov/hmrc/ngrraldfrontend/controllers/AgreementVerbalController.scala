  ...
  def show(mode: Mode): Action[AnyContent] = {
    (authenticate andThen checkRequestSentReference andThen getData).async { implicit request =>
      ...
      Future.successful(Ok(view(
        preparedForm,
        buildRadios(preparedForm, AgreementVerbalForm.agreementVerbalRadio(preparedForm, dateTextFields)),
        ...
        .fold(
          formWithErrors =>
            ...
            Future.successful(BadRequest(view(formWithCorrectedErrors,
              buildRadios(formWithErrors, AgreementVerbalForm.agreementVerbalRadio(formWithCorrectedErrors, dateTextFields)),
              request.property.addressFull,
              mode))
            ),
          agreementVerbalForm =>
